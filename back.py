...
def index():
    ...
    if logged:
        if session["logged"] == True:
            txt = session['name']
            return render_template('espaceclient.html', message=txt, logged=logged)
...
def login():
    connection = engine.connect()
    if request.method == 'POST':
        recup_mail= escape(request.form['mail'])
        recup_mdp = escape(request.form['mdp'])
    #On cherche la correspondance entre la bdd et les données saisies par l'utilisateur
        ...
        resultats = connection.execute(s, x=recup_mail, y=recup_mdp)
        if resultats != None:
